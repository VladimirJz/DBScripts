...
INSERT INTO TMPHISPAGOS 
    ...
    FROM histpagos HIS 
    INNER JOIN #TMPPERIODOS PER on  HIS.id_periodo=PER.id_periodo 


SELECT  concat(CASE WHEN ca.Id_cia =1 THEN 'UPN' ELSE CASE WHEN ca.Id_cia=2 THEN 'BASICA' ELSE 'MANDOS MEDIOS' END END ,
' - ', ca.año*100+ca.numero,' P',CA.PARCIAL,' ',CA.TITULOPERIODO ) QUINCENA, h.id_periodo ID_PERIODO,
ca.leyenda1 NOMBRE_NOMINA,  aa.RFC, aa.CURP,   h.id_emp ID_EMP, h.id_plaza ID_PLAZA,    xx.plazaieepo PLAZA,
h.id_referencia ID_REFERENCIA,  sum(CASE WHEN c.tipo = 1  THEN pesos ELSE 0 end) as  PERCEPCIONES,
